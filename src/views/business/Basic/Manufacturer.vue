<!--
 * @Descripttion: (生产厂家/Manufacturer)
 * @Author: (admin)
 * @Date: (2024-08-30)
-->
<template>
  <div>
    ...
    <el-row :gutter="15" class="mb10">
      ...
      <el-col :span="1.5">
        <el-button type="success" :disabled="single" v-hasPermi="['manufacturer:edit']" plain icon="edit" @click="handleUpdate">
          {{ $t('btn.edit') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" :disabled="multiple" v-hasPermi="['manufacturer:delete']" plain icon="delete" @click="handleDelete">
          {{ $t('btn.delete') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button type="danger" v-hasPermi="['manufacturer:delete']" plain icon="delete" @click="handleClear">
          {{ $t('btn.clean') }}
        </el-button>
      </el-col>
      <el-col :span="1.5">
        <el-dropdown trigger="click" v-hasPermi="['manufacturer:import']">
          ...
          <template #dropdown>
            <el-dropdown-menu>
              <el-dropdown-item command="upload">
                <importData
                  templateUrl="business/Manufacturer/importTemplate"
                  importUrl="/business/Manufacturer/importData"
...
function handleExport() {
  proxy
    .$confirm("是否确认导出生产厂家数据项?", "警告", {
      confirmButtonText: "确定",
      cancelButtonText: "取消",
      type: "warning",
    })
    .then(async () => {
      await proxy.downFile('/business/Manufacturer/export', { ...queryParams })
    })
}
